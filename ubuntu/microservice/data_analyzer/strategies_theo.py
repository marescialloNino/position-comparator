...
from common.paths import ROOT_DIR, PNL_DATA_DIR, PRICES_DIR, OUTPUT_DIR
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(OUTPUT_DIR / 'strategies.log', encoding='utf-8'),
...
class Strategy:
    def __init__(self, name: str, family: str, session: str, config_manager: ConfigManager):
        self.name = name
        self.family = family
        ...
        logger.info(f"Initializing strategy {name} for session {session} with config: {self.config}")
    ...
    def get_aum_and_max_positions(self) -> tuple[float, int]:
        ...
        logger.info(f"Strategy {self.name}: AUM={aum}, Max Positions={max_positions}")
    ...
    def get_price_at_time(self, price_matrix: pd.DataFrame, asset: str, time: datetime) -> Optional[float]:
        ...
        if asset not in price_matrix.columns:
            logger.warning(f"Asset {asset} not found in price matrix for strategy {self.name}")
            return None
        ...
        if deltas[idx] <= timedelta(minutes=120):  # Increased tolerance to 120 minutes
            ...
            if pd.isna(price):
                logger.warning(f"NaN price for {asset} at {time} in strategy {self.name}")
                return None
            logger.debug(f"Price for {asset} at {time}: {price}")
            return price
        logger.warning(f"No price within 120 minutes of {time} for {asset} in strategy {self.name}")
        return None
    ...
    def process_current_state_log(self, price_matrix: pd.DataFrame, output_file: str, closed_file: str):
        ...
        file_path = DATA_DIR / self.session / self.name / "current_state.log"
        output_dir = PNL_DATA_DIR / self.session / "output"
        output_dir.mkdir(parents=True, exist_ok=True)
        if not file_path.exists():
            logger.warning(f"current_state.log not found for session '{self.session}', strategy '{self.name}' at {file_path}")
            return

        # Calculate AUM and max positions
        aum, max_positions = self.get_aum_and_max_positions()
        if aum <= 0 or max_positions <= 0:
            logger.warning(f"Invalid AUM ({aum}) or max positions ({max_positions}) for strategy {self.name}")
            return
        notional_per_position = aum / max_positions  # Absolute value per position
        logger.info(f"Strategy {self.name}: Notional per position={notional_per_position}")
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                ...
                try:
                    ...
                    positions = []
                    if not pos_str:
                        logger.warning(f"Empty position string at line {i+1} in {file_path}")
                        continue
                    items = pos_str.split(',')
                    for item in items:
                        item = item.strip()
                        ...
                        try:
                            ...
                            if direction not in [1, -1]:
                                logger.warning(f"Invalid direction {direction} for {asset} in {file_path}")
                            positions.append((asset, direction))
                        except ValueError:
                            logger.warning(f"Invalid quantity in line {i+1}: {direction}")
                            continue
                    if positions:
                        timestamps.append(timestamp)
                        portfolio_states[timestamp] = positions
                        logger.info(f"Strategy {self.name}: Parsed {len(positions)} positions at {timestamp}")
                    else:
                        logger.warning(f"No valid positions parsed at line {i+1} in {file_path}")
                except Exception as e:
                    logger.warning(f"Failed to parse line in {file_path}: {line}. Error: {e}")
                    continue

        if not timestamps:
            logger.warning(f"No valid entries found in {file_path} for strategy {self.name}")
            return

        logger.info(f"Strategy {self.name}: Parsed {len(timestamps)} timestamps")
        timestamps.sort()
        ...
        for i, ts in enumerate(timestamps):
            ...
            for asset, direction in current_state:
                if asset not in current_positions:
                    ...
                    if entry_price is not None and entry_price > 0:
                        ...
                        logger.warning(f"Skipping position for {asset} at {ts} due to invalid entry price")
            ...
            for asset, pos in current_positions.items():
                ...
                if current_price is None:
                    logger.warning(f"Skipping snapshot for {asset} at {ts} due to missing current price")
                    continue
            ...
            if snapshot:
                ...
                logger.info(f"Strategy {self.name}: Added snapshot at {ts} with {len(snapshot)} positions")
        ...
        logger.info(f"Strategy {self.name}: Generated {len(self.portfolio_snapshots)} snapshots")
        if not self.portfolio_snapshots:
            logger.warning(f"No snapshots generated for strategy {self.name}")
        ...
        logger.info(f"Generated {output_path} and {closed_path} for strategy {self.name}")

class StrategyManager:
    def __init__(self, config_file: str = str(ROOT_DIR / "microservice" / "web_processor.yml")):
        self.config_manager = ConfigManager(config_file)
        self.strategies: Dict[str, Strategy] = {}

    def add_strategy(self, name: str, family: str, session: str):
        self.strategies[name] = Strategy(name, family, session, self.config_manager)
    ...
    def aggregate_account_portfolio(self, account: str, exchange: str, price_matrix: pd.DataFrame):
        ...
        for strat in self.strategies.values():
            logger.debug(f"Checking strategy {strat.name}: active={strat.config.get('active', True)}, "
            ...
            if (strat.config.get('active', True) and
                ...
                if not strat.portfolio_snapshots:
                    logger.warning(f"No portfolio snapshots for strategy {strat.name}")
                    continue
                ...
                try:
                    timestamps = {
                        ...
                        if datetime.strptime(ts, '%Y/%m/%d %H:%M:%S.%f') >= CUTOFF_DATE
                    }
                    all_timestamps.update(timestamps)
                    logger.info(f"Found {len(timestamps)} snapshots for strategy {strat.name}")
                except ValueError as e:
                    logger.warning(f"Error parsing timestamps for strategy {strat.name}: {e}")
                    continue
            else:
                logger.debug(f"Strategy {strat.name} excluded: "
        ...
        if not all_timestamps or not relevant_strategies:
            logger.warning(f"No portfolio snapshots found for account {account} on exchange {exchange}. "
                        f"Relevant strategies: {len(relevant_strategies)}, Timestamps: {len(all_timestamps)}")
            return

        # Aggregate positions at each timestamp
        account_portfolio = {}
        output_dir = PNL_DATA_DIR / exchange.lower() / "output"
        output_dir.mkdir(parents=True, exist_ok=True)
        for ts in sorted(all_timestamps):
            ...
            for asset, quantity in positions.items():
                ...
                if current_price is None:
                    logger.warning(f"No price for {asset} at {ts} for account {account}")
                    continue
        ...
        if not account_portfolio:
            logger.warning(f"No aggregated portfolio entries generated for account {account} on exchange {exchange}")
            return
        ...
        logger.info(f"Generated {output_path} for account {account} on exchange {exchange}")
    ...
    def process_all_current_state_logs(self):
        ...
        logger.info("Processing current_state.log for all strategies:")
        exchanges = set(
        ...
        for strat in self.config_manager.get_active_strategies():
            ...
            session = strat.get('session', exchange)  # Use exchange as session if not specified
            if strat_name not in self.strategies:
                self.add_strategy(strat_name, family, session)
            accounts.add((account, exchange))
            price_matrix = price_matrices.get(exchange)
            if price_matrix is None:
                logger.warning(f"Price matrix not found for exchange '{exchange}', skipping strategy {strat_name}")
                continue
            output_file = f"portfolio_{strat_name}.json"
            closed_file = f"closed_positions_{strat_name}.json"
            logger.info(f"Processing strategy: {strat_name}")
        ...
        for account, exchange in accounts:
            price_matrix = price_matrices.get(exchange)
            if price_matrix is None:
                logger.warning(f"Price matrix not found for exchange '{exchange}', skipping account {account}")
                continue
...
if __name__ == "__main__":
    ...
    for strat in manager.config_manager.get_active_strategies():
        strat_name = strat['name']
        family = strat.get('type', 'basketspread')
        session = strat.get('session', strat.get('exchange_trade', 'bitget'))
        manager.add_strategy(strat_name, family, session)
